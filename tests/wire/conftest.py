...
from payroc.client import Payroc
from payroc.environment import PayrocEnvironment

# Check once at import time whether the client constructor accepts a headers kwarg.
try:
    _CLIENT_SUPPORTS_HEADERS: bool = "headers" in inspect.signature(Payroc).parameters
...
def get_client(test_id: str) -> Payroc:
    """
    ...
    if _CLIENT_SUPPORTS_HEADERS:
        return Payroc(
            environment=PayrocEnvironment(api=base_url, identity=base_url),
